  ...
  for (const [w, s, e, n] of REGIONS) {
    try {
      ...
      for (let k = 1; k < lines.length; k++) {
        ...
        out.push({ id: `firms-${lat.toFixed(3)}-${lng.toFixed(3)}-${c[ti] || k}`, lat, lng, frp: parseFloat(c[fi]) || 0, confidence: conf, date: ((c[di] || '') + ' ' + (c[ti] || '')).trim() });
      }
    } catch (err) { console.error('[firms] region failed', err); }
  }
  out.sort((a, b) => b.frp - a.frp);
  cache = { ts: Date.now(), value: out.slice(0, 600) };
  return cache.value;
}
